...
<div class="page-header">
    ...
    <nav aria-label="breadcrumb">
        <ul class="breadcrumb">
            <li class="breadcrumb">
                <a href="/barang_masuk" class="btn btn-warning btn-sm"><i class="mdi mdi-keyboard-backspace"></i> Back</a>
            </li>
            @if (Auth::user()->role == 1)
            <li class="breadcrumb">
                <a href="/barang_masuk_detail/form/{{ $id }}" class="btn btn-success btn-sm"><i class="mdi mdi-plus"></i> Tambah</a>
            </li>
            @endif
        </ul>
    </nav>
...
<script>
    jQuery(function ($) {
        $('#table').DataTable({
            processing: true,
            serverSide: true,
            ajax: '/barang_masuk_detail/data/{{ $id }}',
            columns: [
